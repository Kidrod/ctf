# Challenge: [ReadMyCert](https://play.picoctf.org/practice/challenge/367)
20 Points
# Description
How about we take you on an adventure on exploring certificate signing requests
Take a look at this CSR file here.
# Solution
This challenge is straightforward to resolve. To address it, I decoded base64 string between -----BEGIN CERTIFICATE REQUEST----- and -----END CERTIFICATE REQUEST----- and found the flag in the ouput of decoding process.

```txt
-----BEGIN CERTIFICATE REQUEST-----

-----END CERTIFICATE REQUEST-----

```
